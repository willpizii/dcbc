...
def coach():
    ...
    if request.method == 'POST':
        code = request.form.get('tfa_code')
    ...
    def generate_qr_code(username, coaches):
        ...
        qr.save(buffer)
        buffer.seek(0)
        return base64.b64encode(buffer.getvalue()).decode('utf-8'), uri

    qr_code_base64, provisioning_uri = generate_qr_code(username, coaches)
    ...
    return(render_template_string('''  {% extends "coachbase.html" %} {% block content %}
            ...
            {% if username %}
                {% if not apped %}
                    ...
                    <p>Alternatively, you can <a href="{{ provisioning_uri }}">click here</a> to add the 2FA setup directly to your mobile app.</p>
                {% else %}
                    <p>Enter the code from your authenticator app:</p>
                {% endif %}
                <form method="post" action="{{ url_for('coaches.coach', username=username) }}">
                    <input type="text" name="tfa_code" required>
                    <input type="hidden" name="username" value="{{ username }}">
                    <button type="submit">Verify</button>
                </form>
            {% endif %}  {% endblock content %}
    ''', username=username, qrcode=qr_code_url, apped=apped, provisioning_uri=provisioning_uri))
